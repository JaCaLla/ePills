...
struct DoseCellTypeView: View {
    ...
    struct Constants {
        static let viewWidth: CGFloat = 35
    }
    ...
    var body: some View {
        ZStack {
            if self.doseCellViewModel.doseCellType == .endToday || self.doseCellViewModel.doseCellType == .endPast {
                VerticalStripeShape2()
                ...
                VerticalStripeShape2()
                    ...
                    .frame(width: Constants.viewWidth)
                RoundedShape()
                    .fill(Color(self.doseCellViewModel.doseCellType == .endToday ? R.color.colorBlueLight.name : R.color.colorBlue.name))
...
struct RoundedShape: Shape {
    ...
    func path(in rect: CGRect) -> Path {
        let bezierPath = UIBezierPath(roundedRect: rect,
                                      ...
                                      cornerRadii: CGSize(width: 35 / 2,
                                                          height: 35 / 2))
        return Path(bezierPath.cgPath)
    }
}

struct VerticalStripeShape2: Shape {
...
struct DoseCellTypeView_Previews: PreviewProvider {
    static var dose: DoseCellViewModel {
        return DoseCellViewModel(doseOrder: "5", day: "2", monthYear: "Junio - 2020", weekdayHHMM: "Viernes - 06:04", realOffset: "-1d 3h", doseCellType: .middle)
    }
