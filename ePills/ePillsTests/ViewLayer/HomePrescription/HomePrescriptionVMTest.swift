...
class HomePrescriptionVMTest: XCTestCase {
    ...
    func test_iconNameWhenNotStarted_en() {
        let prescription = Prescription(name: "a",
                                        unitsBox: 10,
                                        interval: Interval(secs: 8, label: "8 hours"),
                                        unitsDose: 1)
    ...
    func test_getRemainingTimeMessageWhenNotOngoing() {
        ...
        let prescription = Prescription(name: "a",
                                        ...
                                        interval: Interval(secs: 8, label: "8 hours"),
                                        unitsDose: 1)
        XCTAssertEqual(sut.getMajorRemainingTimeMessage(prescription: prescription, timeManager: timeManager).0, "")
        XCTAssertEqual(sut.getMajorRemainingTimeMessage(prescription: prescription, timeManager: timeManager).1, "")
    ...
    func test_getRemainingTimeMessageWhenNextIsGreaterThanAMonth() {
        ...
        let prescription = Prescription(name: "a",
                                        ...
                                        unitsDose: 1)
        prescription.takeDose(timeManager: timeManager)
        XCTAssertEqual(sut.getMajorRemainingTimeMessage(prescription: prescription, timeManager: timeManager).0, "> Month")
        XCTAssertEqual(sut.getMajorRemainingTimeMessage(prescription: prescription, timeManager: timeManager).1, "")
    ...
    func test_getRemainingTimeMessageWhenNextIsGreaterThanDays() {
        ...
        let prescription = Prescription(name: "a",
                                        ...
                                        unitsDose: 1)
        prescription.takeDose(timeManager: timeManager)
        XCTAssertEqual(sut.getMajorRemainingTimeMessage(prescription: prescription, timeManager: timeManager).0, "-1d")
        XCTAssertEqual(sut.getMajorRemainingTimeMessage(prescription: prescription, timeManager: timeManager).1, "01h")
        
        prescription.nextDose = nil
        prescription.interval = Interval(secs: 3600 * 24 * 1, label: "1 day")
        prescription.takeDose(timeManager: timeManager)
        XCTAssertEqual(sut.getMajorRemainingTimeMessage(prescription: prescription, timeManager: timeManager).0, "-1d")
        XCTAssertEqual(sut.getMajorRemainingTimeMessage(prescription: prescription, timeManager: timeManager).1, "00h")
    }
    ...
    func test_getRemainingTimeMessageWhenNextIsGreaterThanHours() {
        ...
        let prescription = Prescription(name: "a",
                                        ...
                                        unitsDose: 1)
        prescription.takeDose(timeManager: timeManager)
        XCTAssertEqual(sut.getMajorRemainingTimeMessage(prescription: prescription, timeManager: timeManager).0, "-23h")
        XCTAssertEqual(sut.getMajorRemainingTimeMessage(prescription: prescription, timeManager: timeManager).1, "59m")
    }
    ...
    func test_getRemainingTimeMessageWhenNextIsGreaterThanMins() {
        ...
        let prescription = Prescription(name: "a",
                                        ...
                                        unitsDose: 1)
        prescription.takeDose(timeManager: timeManager)
        XCTAssertEqual(sut.getMajorRemainingTimeMessage(prescription: prescription, timeManager: timeManager).0, "-59m")
        XCTAssertEqual(sut.getMajorRemainingTimeMessage(prescription: prescription, timeManager: timeManager).1, "59s")
    }
    ...
    func test_getRemainingTimeMessageWhenNextIsGreaterThanSecs() {
        ...
        let prescription = Prescription(name: "a",
                                        ...
                                        unitsDose: 1)
        prescription.takeDose(timeManager: timeManager)
        XCTAssertEqual(sut.getMajorRemainingTimeMessage(prescription: prescription, timeManager: timeManager).0, "-30s")
        XCTAssertEqual(sut.getMajorRemainingTimeMessage(prescription: prescription, timeManager: timeManager).1, "")
    }
    ...
    func test_getRemainingTimeMessageWhenNextEllapsedThanSecs() {
        ...
        XCTAssertEqual(sut.getMajorRemainingTimeMessage(prescription: prescription, timeManager: timeManager).0, "15s")
        XCTAssertEqual(sut.getMajorRemainingTimeMessage(prescription: prescription, timeManager: timeManager).1, "")
    }
   ...
   func test_getRemainingTimeMessageWhenNextEllapsedThanMins() {
       ...
       XCTAssertEqual(sut.getMajorRemainingTimeMessage(prescription: prescription, timeManager: timeManager).0, "05m")
       XCTAssertEqual(sut.getMajorRemainingTimeMessage(prescription: prescription, timeManager: timeManager).1, "10s")
   }
    ...
    func test_getRemainingTimeMessageWhenNextEllapsedThanHours() {
        ...
        XCTAssertEqual(sut.getMajorRemainingTimeMessage(prescription: prescription, timeManager: timeManager).0, "05h")
        XCTAssertEqual(sut.getMajorRemainingTimeMessage(prescription: prescription, timeManager: timeManager).1, "03m")
    }
    ...
    func test_getRemainingTimeMessageWhenNextEllapsedThanDays() {
        ...
        XCTAssertEqual(sut.getMajorRemainingTimeMessage(prescription: prescription, timeManager: timeManager).0, "02d")
        XCTAssertEqual(sut.getMajorRemainingTimeMessage(prescription: prescription, timeManager: timeManager).1, "00h")
    }
